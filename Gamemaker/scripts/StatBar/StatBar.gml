...
function StatBar(_config = {}) constructor {
	...
	static render = function(_stat_previous, _stat_current, _stat_max, _event_number) {
		...
		// TODO Get scale from resolution manager
		var _scale = (_event_number == ev_gui) ? window_get_width() / 480 : 1;

        // Set shader + uniforms
        shader_set(__shader);
        shader_set_uniform_f(__u_bright,    _bright[0], _bright[1], _bright[2]);
	    shader_set_uniform_f(__u_dark,      _dark[0],   _dark[1],   _dark[2]);
	    shader_set_uniform_f(__u_fill,      _pct);
	    shader_set_uniform_f(__u_bar_x,     __.x);
	    shader_set_uniform_f(__u_bar_width, __.width);
	    shader_set_uniform_f(__u_scale,     _scale);

        // Draw fill
        draw_sprite_stretched(spr_stat_bar_fill, 0, __.x + 1, __.y + 1, __.width - 2, __.height - 2);
