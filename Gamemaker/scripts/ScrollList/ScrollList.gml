...
function ScrollList(_config) : UIParent(_config) constructor {
    ...
    with (__) {
        ...
        line_height = _config[$ "line_height"] ?? 8; // TODO Move to ui manager

        // Content / surface
        content_size = 0;   // total scrollable extent (height OR width)
        surface      = -1;
		...
		// TODO Implement proper constructors for the pseudo elements, good use of static functions
    ...
    static init = function() {
		...
		// TODO Implement proper children integration
        ...
        for (var _i = 0; _i < _count; _i++) {
            ...
            // var _item_main     = (_i % 2 == 0) ? 60 : 40;
			var _item_main = 72;

            var _item_w = _is_vertical ? (_item_cross - 1) : (_item_main - 1);
            var _item_h = _is_vertical ? (_item_main  - 1) : (_item_cross - 1);
    ...
    static step = function() {
        var _is_vertical  = (__.scroll_axis == ScrollAxis.Vertical);
        var _view_size    = _is_vertical ? __.height : __.width;   // visible extent along main axis
    ...
    static rebuild_content = function() {
        ...
        // Content_size = total extent along the main axis
        var _view_size    = (_is_vertical) ? __.height + _border_offset : __.width + _border_offset;
        ...
        if (__.content_size > _view_size) {
            for (var _i = 0; _i < _count; _i++) {
                var _item = __.children[_i];
                if (_is_vertical) {
					var _w = _item.get_width() - __.scrollbar_thickness;
					_item.set_width(_w);
                } else {
					var _h = _item.get_height() - __.scrollbar_thickness;
                    _item.set_height(_h);
                }
            }
        }
